...
FAILED_INITIALIZATION_ERROR_MSG = "The initialized work has not yet passed the self-inspection, so it's not allowed to be executed!"
...
class ExecutionEntity:
    ...
    def locate(self, relative_locator: list) -> ExecutionEntity:
        ...
        if len(relative_locator) == 0:
            ...
            try:
                ...
                LOGGER.warning(f"Unable to find the entity with locator {relative_locator} from {self.identifier}.")
                return None
    ...
    def deprecate_and_delete(self) -> None:
        ...
        """
        self.status = StatusCode.DEPRECATED
    ...
    def dump_snapshot_file(self, save_directory: str = str()) -> str:
        ...
        if (path.exists(save_directory)):
            ...
            LOGGER.warning(f"The `save_directory` value '{save_directory}' you specified is not a directory, so change the save address to working_directory.")
        else:
            LOGGER.info("No specified directory to save the Pickle file. Save it to the working directory.")
        ...
        try:
            ...
            if dir_to_save != self.working_directory:
                LOGGER.warning(f"Unable to save file to '{dir_to_save}'. Using your working directory '{self.working_directory}' instead.")
                dir_to_save = self.working_directory
                filepath = path.join(dir_to_save, filename)
                with open(file=filepath, mode="wb") as fobj:
                    pickle.dump(self, file=fobj)
    ...
    def reload(self):
        ...
        if self.status in StatusCode.unexecuted_statuses:
            LOGGER.info(f"{self.identifier} hasn't been executed yet, cannot reload!")
            return
        LOGGER.info(f"Reloading {self.identifier}...")
...
class WorkFlow(ExecutionEntity):
    ...
    def __init__(self, working_directory: str = BASE_DIRECTORY, 
        ...
        LOGGER.debug(f"outer_data_mappers: {self.outer_data_mappers}")
    ...
    def execute(self):
        ...
        """
        if not self.is_executable:
            LOGGER.error(f"{self.identifier}: {FAILED_INITIALIZATION_ERROR_MSG}")
            return dict()
        else:
            if not self.control_workunit:    # If the current workflow is at the outermost level, log the message; otherwise skip the duplicated message.
                LOGGER.info(SUCCESS_INITIALIZATION_MSG)
            self.status = StatusCode.RUNNING
            for workunit in self.workunits:
                workunit: WorkUnit
                LOGGER.info(f"Executing WorkUnit {workunit.identifier} ...")
    ...
    def deprecate_and_delete(self) -> None:
        """Deprecate and Delete the WorkFlow instance as well as its internal workunits.
        Its record will be removed from the database.
        """
        for workunit in self.workunits:
            workunit.deprecate_and_delete()
...
class WorkUnit(ExecutionEntity):
    ...
    Attributes:
        api_key (str): Key used to identify and map the appropriate basic API function.
    ...
    def from_dict(cls, unit_dict: dict, workflow: WorkFlow = None, working_directory: str = BASE_DIRECTORY, locator: List[int] = list(), sqlite_filepath: str = str(), debug: bool = False) -> WorkUnit:
        ...
        # Map the API.
        unit.api = UNIT_API_MAPPER.get(unit.api_key, None)
        ...
        LOGGER.debug(f"The API of '{unit.identifier}' is '{unit.api}'.")
        # Self Inspection.
    ...
    def reload(self, unit_dict: dict, api: Callable = None) -> WorkUnit:
        ...
        if (api is None):
            virtual_unit.api = UNIT_API_MAPPER.get(virtual_unit.api_key)
        else:
    ...
    def execute(self) -> Tuple[str, Any]:
        ...
        try:
            ...
            LOGGER.debug(f"{self.identifier} with Status {self.status}.")
            ...
            if (self.status in StatusCode.skippable_statuses):
                LOGGER.info(f"WorkUnit {self.identifier} is skipped!")
                return self.return_key, self.return_value
            ...
            if self.return_key:
                ...
                self.return_value = None
            self.status = StatusCode.EXIT_OK
            LOGGER.debug(f"{self.identifier}(return_key={self.return_key}, return_value={self.return_value})")
            return self.return_key, self.return_value
        except Exception as exc:
            LOGGER.error(f'Catching Error when executing `{self.identifier}`:')
    ...
    def deprecate_and_delete(self) -> None:
        """Deprecate and Delete the WorkUnit instance and remove its record from the database."""
        super().deprecate_and_delete()
    ...
    def __inspect_argument_type(arg_value: Any, annotation: Union[Type, Any]) -> bool:
        ...
        if (annotation == Any):
            ...
            if (annotation.__origin__ is Union):   # Handle Union Type.
                ...
                LOGGER.debug(f"Type from Union are {valid_types}.")
                ...
                if (subscripted_generics):
                    LOGGER.warning(f"Please make sure that `{arg_value}` is an instance of {' or '.join(subscripted_generics)}.")
                    return True
                else:
                    return False
            else:
            ...
            if (not isinstance(annotation, type)):
                ...
                if (isinstance(annotation, str)):
                    ...
                    LOGGER.warning(f"Unable to determine if `{arg_value}` is an instance of {annotation}. Please ensure it by yourself")
                    return True
    ...
    def __load_data_mapper_value(self, param_name: str, arg_value: str) -> Tuple[bool, Any]:
        ...
        for mapper in data_mappers:    # Listed in order of priority.
            if (mapper and unformatted_arg_value in mapper):    # If mapped, leave for later.
                ...
                LOGGER.debug(f"Checkpoint 1: The mapped value is `{mapped_value}`.")
                if (mapped_value != None):   # Check if there is a non-none-or-empty value in Mapper.
                    LOGGER.debug(f"Checkpoint 2: The mapped value `{mapped_value}` is not NoneValue.")
                    if (isinstance(mapped_value, type(Placeholder.PLACEHOLDER_STR_VALUE)) # Placeholder values need to be excluded.
                        ...
                        LOGGER.debug(f"Checkpoint 3: The mapped value `{mapped_value}` is a placeholder value.")
                        break
        ...
        if (is_key_mapped and is_value_mapped_none):    # If the key is mapped but the value is still None, skip inspection.
            ...
            self.error_msg_list.append(error_msg)
            return True, arg_value
        LOGGER.debug(f"{self.api_key}, param_name: {param_name}, arg_value: {unformatted_arg_value}, Value Changed.")
    ...
    def self_inspection_and_args_reassembly(self) -> None:
        ...
        for param_name in api_parameters:
            ...
            # Load data from `intermediate_data_mapper` or `inherited_data_mapper`.
            LOGGER.debug(f"{self.api_key}, param_name: {param.name}, arg_value: {arg_value}")
...
class IterativeWorkUnit(ControlWorkUnit):
    ...
    def reload(self, unit_dict: dict, placeholder_api: Callable, iterable_data_label: str, iterative_body_datum_label: str):
        ...
        if (iterative_data:=self.args_dict_to_pass_backup.get(iterable_data_label)) == reloaded_iterative_data:
            ...
            LOGGER.warning(f"Inconsistent {iterable_data_label} in {self.identifier}, all the workflows are to be cleared.")
            
            [sub_workflow.deprecate_and_delete() for sub_workflow in self.sub_workflows]
...
class LoopWorkUnit(IterativeWorkUnit):
    ...
    def execute(self) -> Tuple[str, Any]:
        ...
        self.update_args_value_from_data_mapper()
        LOGGER.info(f'We are about to execute {self.__class__.__name__} {self.identifier} ...')
        self.status = StatusCode.RUNNING
        ...
        for loop_index, workflow in enumerate(self.sub_workflows):
            LOGGER.debug(f'Executing {workflow.identifier} ...')
            ...
            if (self.run_in_subfolders):
                ...
                if (not use_existing_subflows):
                    remove_directory(flow_working_subfolder, empty_only=False)
                make_directory(flow_working_subfolder)
...
class GeneralWorkUnit(ControlWorkUnit):
    ...
    def from_dict(cls, unit_dict: dict, working_directory: str = BASE_DIRECTORY, 
        ...
        if not save_snapshot:
            ...
            if not debug:
                sleep_seconds = 5.0
                LOGGER.warning(f"Press CTRL+C to cancel the job now, or we continue in {sleep_seconds} seconds...\n")
                sleep(sleep_seconds)
    ...
    def execute(self) -> Tuple[str, Any]:
        ...
        """
        if not self.is_executable:
            LOGGER.error(FAILED_INITIALIZATION_ERROR_MSG)
            ...
            if (self.save_snapshot):
                ...
                LOGGER.info(f"The current running status has been saved to '{pickle_filepath}'. Please go to this location to view your file(s).")
            return self.return_key, self.return_value
    ...
    def reload(self, unit_dict: dict, data_mapper_for_reload: dict = dict()) -> None:
        ...
        for reload_mapper_key, reload_mapper_value in data_mapper_for_reload.items():
            if self.data_mapper_for_init.get(reload_mapper_key) != reload_mapper_value:
                # If not all the items in `data_mapper_for_reload` is already in `self.data_mapper_for_init`, then updates exist.
                self.data_mapper_for_init[reload_mapper_key] = reload_mapper_value
                if self.status != StatusCode.FAILED_INITIALIZATION:
                    self.status = StatusCode.SUSPECIOUS_UPDATES 
